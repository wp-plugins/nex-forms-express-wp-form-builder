...
jQuery('#the_form_title').popover({trigger:'manual'});
	jQuery('#save_nex_form').live('click',
		function()
			...
			jQuery('div.nex-forms-container div.cd-dropdown').remove();
	...
	function load_nex_event_calendars(cal_id){
		...
		jQuery.post
			...
			ajaxurl, data, function(response)
				...
				if(jQuery('.form_update_id').text())
					jQuery('.saved_forms a#'+jQuery('.form_update_id').text()).addClass('active');
				}
			);
		}
...
jQuery('.delete_the_calendar').live('click',
		function()
			{		
			jQuery(this).closest('a').find('.do_permanent_delete').addClass('opened');
			}
		);
...
jQuery('.dont_delete, .do_delete').live('click',
		function()
			{		
			jQuery(this).closest('a').find('.do_permanent_delete').removeClass('opened');
			}
		);
...
jQuery('.do_delete').live('click',
		function()
			{
			if(jQuery(this).attr('id') == jQuery('.form_update_id').text())
				{
				jQuery('.form_update_id').text('');
				jQuery('#the_form_title').val('');
			...
			jQuery(this).closest('a').fadeOut('slow');
...
jQuery('.embed_form').live('click',
		function()
			...
			if(jQuery('.form_update_id').text()=='')
				...
				jQuery(this).popover('destroy');
				jQuery('#useForm').find('.sc_normal').text('[NEXForms id="'+ jQuery('.form_update_id').text() +'"]');
				jQuery('#useForm').find('.sc_popup_button').text('[NEXForms id="'+ jQuery('.form_update_id').text() +'" open_trigger="popup" type="button" text="Open Form"]');
				jQuery('#useForm').find('.sc_popup_link').text('[NEXForms id="'+ jQuery('.form_update_id').text() +'" open_trigger="popup" type="link" text="Open Form"]');
				
				jQuery('#useForm').find('.php_normal').html('&lt;?php NEXForms_ui_output(array("id"=>'+ jQuery('.form_update_id').text() +'),true); ?&gt;');
				jQuery('#useForm').find('.php_popup_button').html('&lt;?php NEXForms_ui_output(array("id"=>'+ jQuery('.form_update_id').text() +',"open_trigger"=>"popup", "type"=>"button", "text"=>"Open Form"),true); ?&gt;');
				jQuery('#useForm').find('.php_popup_link').html('&lt;?php NEXForms_ui_output(array("id"=>'+ jQuery('.form_update_id').text() +',"open_trigger"=>"popup", "type"=>"link", "text"=>"Open Form"),true); ?&gt;');
				}
			}
		);
...
/*************************************************************************************/			
	jQuery('.saved_forms .calendar_title').live('click',
		function()
			...
			var the_ID = jQuery(this).closest('a').attr('id');
