...
(function($)
	...
	$(document).ready
		...
		function()
			...
			$('.add_paypal_product').click(
				function()
					...
					pp_clone.find('.product_number').text('Item '+ $('.paypal_products .paypal_product').size());
			...
			$('.remove_paypal_product').live('click',
				function()
					...
					$('.paypal_products .paypal_product').each(
						function(index)
							{
							$(this).find('.product_number').text('Item '+ (index+1));
							}
						);
					}
				);
		...
		jQuery('div.nex-forms-container label, div.nex-forms-container label .label_container, div.nex-forms-container div.form_field.submit-button, div.nex-forms-container  label small, .field_settings .edit').live('click',function(){ if(!jQuery('div.nex-forms-container').hasClass('selecting_conditional_target')) { jQuery('a.the-label').trigger('click').tab('show'); setTimeout(function(){jQuery('a.the-label').trigger('click').tab('show'); },100) } });
		...
		jQuery('div.nex-forms-container div.input-inner .help-block, div.nex-forms-container div.input-inner label .the_label .bs-tooltip').live('click',function(){  if(!jQuery('div.nex-forms-container').hasClass('selecting_conditional_target')) { jQuery('#filters li:eq(2) a').trigger('click').tab('show'); setTimeout(function(){jQuery('#filters li:eq(2) a').trigger('click').tab('show'); },100)} });
		jQuery('.field_settings .logic').live('click',function(){  if(!jQuery('div.nex-forms-container').hasClass('selecting_conditional_target')) { jQuery('#filters li:eq(4) a').trigger('click').tab('show'); setTimeout(function(){jQuery('#filters li:eq(4) a').trigger('click').tab('show'); },100) } });
...
jQuery('button.do_import').click(
		function(e)
			{
			e.preventDefault();
				jQuery('form.import_form').submit();
				console.log('clicked');
			}
		);
	 
jQuery('form.import_form').ajaxForm({
    data: {
       action: 'import_nex_form'
    },
    //dataType: 'json',
    beforeSubmit: function(formData, jqForm, options) {	 
		 console.log('before');
...
/*** PRE-POSTFIX ****/	
			...
			$('.dropdown-menu.prefix-color li a').click(
				function()
					...
					current_class = current_class.replace('nf-default','');
		    ...
		    var animation_delay = $('#'+current_id).attr('data-wow-delay');
			if(animation_delay)
				{
				animation_delay = animation_delay.replace('s','')
				$('div#nex-forms-field-settings #animation_delay').val(animation_delay)
				}
			else
				$('div#nex-forms-field-settings #animation_delay').val('')
			$('div#nex-forms-field-settings #animation_delay').keyup(
				function()
					{
					$('#'+current_id).attr('data-wow-delay', $(this).val()+'s');
					}
				);
			
			var animation_duration = $('#'+current_id).attr('data-wow-duration');
			if(animation_duration)
				{
				animation_duration = animation_duration.replace('s','')
				$('div#nex-forms-field-settings #animation_duration').val(animation_duration)
				}
			else
				$('div#nex-forms-field-settings #animation_duration').val('')
			$('div#nex-forms-field-settings #animation_duration').keyup(
				function()
					{
					$('#'+current_id).attr('data-wow-duration', $(this).val()+'s');
					}
				);
		   
		   $('div#nex-forms-field-settings #field_animation option[value="no_animation"]').attr('selected','selected');
		   $('div#nex-forms-field-settings #field_animation option').each(
				function()
					{
					if($('#'+current_id).hasClass($(this).text()))
						$(this).attr('selected','selected');
					}
				);
		   
		   $('div#nex-forms-field-settings #field_animation').change(
				function()
					{
					var element = $('#'+current_id);
					
					var animation_preview = $('.animation_preview')
					animation_preview.attr('class','');
					
					animation_preview.addClass('animation_preview');
					animation_preview.addClass($(this).val());
					animation_preview.addClass('animated');
					
					$('div#nex-forms-field-settings #field_animation option').each(
						function()
							{
							element.removeClass($(this).text());
							}
					)
					
					
				if($(this).val()=='no_animation')
					element.removeClass('wow');
				else
					element.addClass('wow').addClass($(this).val());
					}
				);
/***************/					
/*** LABELS ****/			
					...
					$('#'+current_id).find('div span.check-label').each(
						function()
							...
							if($(this).html()!=$(this).parent().find('input').val())
								current_inputs += $(this).parent().find('input').val()+'=='+$(this).html() +'\n';	
							else
								current_inputs += $(this).html() +'\n';	
							}
						);	
					}
				else
...
/* FIELD INPUT */	
			...
			$('.required-star button').click(
				function()
					...
					if($(this).hasClass('empty'))
						...
						get_obj.parent().find('.is_required').removeClass('glyphicon-star-empty').removeClass('glyphicon-star').removeClass('glyphicon-asterisk');
					else
		...
		//PLACE HOLDER
			$('div#nex-forms-field-settings #set_input_id').val($('#'+current_id).find('.the_input_element').attr('id'))
			$('div#nex-forms-field-settings #set_input_id').keyup(
				function()
					{
					$('#'+current_id).find('.the_input_element').attr('id', $(this).val());
					}
				);
		//INPUT ID
...
//INPUT
			$('.overall-input-size button').click(
				function()
					{
					$('.overall-input-size button').removeClass('btn-primary');
					$(this).addClass('btn-primary');
					
					var set_class = '';
					
					jQuery('#collapseFormsCanvas .form_field .fileinput .form-control').removeClass('nf-input-sm').removeClass('nf-input-med').removeClass('nf-input-lg').removeClass('nf-input-xlg');
					jQuery('#collapseFormsCanvas .form_field .bootstrap-tagsinput').removeClass('nf-input-sm').removeClass('nf-input-med').removeClass('nf-input-lg').removeClass('nf-input-xlg');
					jQuery('#collapseFormsCanvas .form_field .selectpicker.btn').removeClass('nf-input-sm').removeClass('nf-input-med').removeClass('nf-input-lg').removeClass('nf-input-xlg');
					jQuery('#collapseFormsCanvas .form_field input.form-control').removeClass('nf-input-sm').removeClass('nf-input-med').removeClass('nf-input-lg').removeClass('nf-input-xlg');
					
					if($(this).hasClass('small'))
						set_class = 'nf-input-sm';
					if($(this).hasClass('normal'))
						set_class = 'nf-input-med';
					if($(this).hasClass('large'))
						set_class = 'nf-input-lg';
					if($(this).hasClass('x-large'))
						set_class = 'nf-input-xlg';
					
						jQuery('#collapseFormsCanvas  .form_field .fileinput .form-control').addClass(set_class);
						jQuery('#collapseFormsCanvas  .form_field .bootstrap-tagsinput').addClass(set_class);
						jQuery('#collapseFormsCanvas  .form_field .selectpicker.btn').addClass(set_class);
						jQuery('#collapseFormsCanvas  .form_field input.form-control').addClass(set_class);
					}
				);
				
		
			$('.dropdown-menu.overall-prefix-color li a').click(
				function()
					{
					$('.dropdown-menu.overall-prefix-color li').removeClass('selected')
					$(this).parent().addClass('selected');
					
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('nf-default');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('alert-danger');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('alert-warning');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('alert-info');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('alert-success');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('label-danger');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('label-warning');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('label-primary');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('label-info');
					$('#collapseFormsCanvas .input-group-addon.prefix').removeClass('label-success');					
					$('#collapseFormsCanvas .input-group-addon.prefix').addClass($(this).attr('class'));
					}
				);	
				
			$('.dropdown-menu.overall-postfix-color li a').click(
				function()
					{
					$('.dropdown-menu.overall-postfix-color li').removeClass('selected')
					$(this).parent().addClass('selected');
					
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('nf-default');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('alert-danger');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('alert-warning');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('alert-info');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('alert-success');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('label-danger');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('label-warning');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('label-primary');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('label-info');
					$('#collapseFormsCanvas .input-group-addon.postfix').removeClass('label-success');					
					$('#collapseFormsCanvas .input-group-addon.postfix').addClass($(this).attr('class'));
					}
				);	
	...
	//CATEGOTRIZE IT
	 var $container = jQuery('#categorize_it_container');
	  ...
	  $container.categorize_it({
		itemSelector : '.col-xs-6,.col-sm-6, .col-xs-4,.col-xs-8 , .col-sm-12, .col-xs-12'
	  });
