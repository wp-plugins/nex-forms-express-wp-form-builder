...
function NEXForms_run_instalation(){
	...
	$extra2_instalation = new IZC_Instalation();
	$extra2_instalation->component_prefix 		=  $config->plugin_prefix;
	$extra2_instalation->component_alias		=  'nex_forms_stats';
	$extra2_instalation->db_table_fields		=  $config->stats_table_fields;
	$extra2_instalation->db_table_primary_key	=  $config->plugin_db_primary_key;
	$extra2_instalation->install_component_table();	
	...
	IZC_Database::alter_plugin_table('wap_nex_forms','time_viewed','text');
...
function NEXForms_main_page(){
	...
	.top-strip  button:hover span.fa, .top-strip  button:hover span.glyphicon, .top-strip  button.save_form span.glyphicon{
	...
	global $wp_styles;
	$include_style_array = array('colors','wp-admin','login','install','wp-color-picker','customize-controls','customize-widgets','press-this','ie','buttons','dashicons','open-sans','admin-bar','wp-auth-check','editor-buttons','media-views','wp-pointer','imgareaselect','wp-jquery-ui-dialog','mediaelement','wp-mediaelement','thickbox','media','farbtastic','jcrop','colors-fresh','nex-forms-admin-holy-grail.min','nex-forms-bootstrap.min','nex-forms-bootstrap-fields','nex-forms-font-awesome','nex-forms-admin','nex-forms-ui','nex-forms-jQuery-UI','defaults','styles-chosen','styles-font-menu','nex-forms-animate');
...
function NEXForms_ui_output( $atts , $echo=''){
	...
	if(is_array($atts))
		...
		//$update_stats = $wpdb->query($stats_entry);
		...
		$output .= '<div id="nex-forms" class="nex-forms">';
			...
			if($form_attr->post_action!='ajax')
				...
				$output .= 	'<form id="" class="'.$set_ajax.'" name="nex_form" action="'.$post_action.'" method="'.$post_method.'" enctype="multipart/form-data">';
					...
					if(!$form_attr->total_views)
						$form_attr->total_views = 0;
						
					$form_views = $form_attr->total_views;
					
					$form_views++;
					$wpdb->insert($wpdb->prefix.'wap_nex_forms_stats',
										array(								
											'time_viewed'				=> mktime(),
